...
def login(request: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if not authentication.Hash.verify_password(request.password,user.hashed_password):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, details=f"Incorrect Password")

    access_token = authentication.create_access_token(data={"username": user.username, "id": user.id})
